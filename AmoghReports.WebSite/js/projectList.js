//var selectedMenu = "Reports";
$(document).ready(function () {
   
    $('#dataTable tbody tr').click(function () {
        $(this).addClass('bg-success').siblings().removeClass('bg-success');
    });

    var getHighlightRow = function () {
        return $('table > tbody > tr.bg-success');
    }

    //$("#btnLogOut").bind("click", function () {
    //    $.ajax({
    //        type: "POST",
    //        //url: '/Home/LogOut',
    //        url: document.getElementById('hdnURL').value + "Home/LogOut",
    //        contentType: "application/json; charset=utf-8",
    //        dataType: "html",
    //        success: function (response) {
              
    //        },
    //        error: function (xhr, status, error) {
    //        }
    //    }); 
    //});

    

    //$("#accordionSidebar li a").click(function (e) {
    //    e.preventDefault();
    //    selectedMenu = $(this).find('span').html();
    //});


    $("#dataTable").on('click', function () {
        var projectName;
        var rows = getHighlightRow();
        projectName = rows[0].cells[2].innerText;
        if (projectName == "GC32") {
            projectName = 1;
        }
        if (projectName == "DUQM") {
            projectName = 2;
        }
        if (projectName == "BPCL") {
            projectName = 3;
        }
        if (projectName == "BIFP") {
            projectName = 4;
        }
        if (projectName == "TINRHERT") {
            projectName = 5;
        }
        if (projectName == "OC1 Petrofac") {
            projectName = 6;
        }
        if (projectName == "OC2 Saipem") {
            projectName = 7;
        }
        if (projectName == "OC3 Samsung") {
            projectName = 8;
        }
        sessionStorage.setItem("reportID", projectName);
        //window.location = document.getElementById('hdnURL').value + "Home/ReportsList?ProjId=" + projectName + "&menu=" + selectedMenu; 
        window.location = document.getElementById('hdnURL').value + "Home/ReportsList?ProjId=" + projectName; 
    });   

    //$('#myReports tbody').on('click', 'tr', function () {
    //    var table = $('#myReports').DataTable();
    //    var data = table.row(this).data();
    //    window.open('../../Reports/WeldingSummaryReport.aspx', '_blank');
    //    //window.open("/Home/WeldingSummaryReport", 'WeldingReport', 'height=500,width=700');
    //});

    
});



