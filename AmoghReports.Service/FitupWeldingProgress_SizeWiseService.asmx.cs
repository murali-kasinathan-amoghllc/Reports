...
{
    /// <summary>
    /// Summary description for FitupWeldingProgress_SizeWiseService
    /// </summary>
    [WebService(Namespace = "http://maakuthari.com/")]
    [WebServiceBinding(ConformsTo = WsiProfiles.BasicProfile1_1)]
    [System.ComponentModel.ToolboxItem(false)]
    [System.Web.Script.Services.ScriptService]
    public class FitupWeldingProgress_SizeWiseService : System.Web.Services.WebService
    {

        [WebMethod]
        [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
        public void GetFitupWeldingProgress_SizeWiseDetails(int ProjId, string CAT_ID, int SUB_CON_ID, string DATE_FROM, string DATE_TO, string IsShop)
        {
            Context.Response.Clear();
            Context.Response.ContentType = "application/json";
            List<FitupWeldingProgress_SizeWise_Model> fitupWeldingProgressList = new List<FitupWeldingProgress_SizeWise_Model>();
            ...
            using (OracleConnection conn = new OracleConnection(conn_str))
            {
                ...
                if (IsShop.ToUpper() == "Y")
                {
                    string query = "SELECT JOB_CODE, JOB_NAME, PROJ_REM, SUB_CON_ID, " +
                     $"SUB_CON_NAME, JOINT_SIZE, JOINT_SIZE_DEC," +
                     ...
                     $"GROUP BY JOB_CODE, JOB_NAME, PROJ_REM, JOINT_SIZE, SUB_CON_ID, SUB_CON_NAME, JOINT_SIZE_DEC " +
                     $"ORDER BY SUB_CON_NAME, JOINT_SIZE_DEC";
                     sql = query;
                }
                else
                {
                    string query = "SELECT JOB_CODE, JOB_NAME, PROJ_REM, FIELD_SC_ID, " +
                      ...
                      $"GROUP BY JOB_CODE, JOB_NAME, PROJ_REM, JOINT_SIZE, FIELD_SC_ID, SUB_CON_NAME, JOINT_SIZE_DEC " +
                      $"ORDER BY SUB_CON_NAME, JOINT_SIZE_DEC ";
                    sql = query;
                }
                ...
                using (OracleDataReader dr = cmd.ExecuteReader())
                {
                    while (dr.Read())
                    {
                        FitupWeldingProgress_SizeWise_Model fitupWeldingProgress = new FitupWeldingProgress_SizeWise_Model();
                        ...
                        fitupWeldingProgress.JOINT_SIZE = dr["JOINT_SIZE"].ToString();
                        fitupWeldingProgress.JOINT_SIZE_DEC = MathUtility.stringToDouble(dr["JOINT_SIZE_DEC"].ToString());
        ...
        [WebMethod]
        [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
        public void GetFitupWeldingProgress_SizeWiseDetailsPDF(int ProjId, string CAT_ID, int SUB_CON_ID, string DATE_FROM, string DATE_TO, string IsShop)
        {
            Random random = new Random();
            int fileId = random.Next(1000, 9999);
            string fileName = $"FitupWeldingProgress_SizeWise_{ fileId}.pdf";
