...
{
    /// <summary>
    /// Summary description for WelderPerformanceLineClassLengthService
    /// </summary>
    [WebService(Namespace = "http://maakuthari.com/")]
    [WebServiceBinding(ConformsTo = WsiProfiles.BasicProfile1_1)]
    [System.ComponentModel.ToolboxItem(false)]
    [System.Web.Script.Services.ScriptService]
    public class WelderPerformanceLineClassLengthService : System.Web.Services.WebService
    {

        [WebMethod]
        [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
        public void GetWelderPerformanceLineClassLengthDetails(int ProjId, int SUB_CON_ID, string DATE_FROM, string DATE_TO)
        {
            ...
            using (OracleConnection conn = new OracleConnection(conn_str))
            {
                ...
                string sql = "SELECT PROJECT_ID, JOB_CODE, CLIENT_NAME, JOB_NAME, PROJ_DESC, " +
                      $"SC_ID, SUB_CON_NAME, WELDER_ID, WELDER_NO, REP_TITLE1, REP_TITLE2," +
                      $"SUM(FNC_IIF_DATE(CREATE_DATE, '{DATE_TO}', WELD_LEN, WELD_LEN, 0)) AS JNTS_WELDED_C," +
                      $"SUM(FNC_IIF_DATE(CREATE_DATE, '{DATE_TO}', RT_LEN, RT_LEN, 0)) AS JNTS_RT_DONE_C," +
                      $"SUM(FNC_IIF_DATE(CREATE_DATE, '{DATE_TO}', REP_LEN, REP_LEN, 0)) AS JNTS_REPAR_C," +
                      $"SUM(FNC_BETWEEN_DATE('{DATE_FROM}', '{DATE_TO}', CREATE_DATE, WELD_LEN)) AS JNTS_WELDED_THIS," +
                      $"SUM(FNC_BETWEEN_DATE('{DATE_FROM}', '{DATE_TO}', CREATE_DATE, RT_LEN)) AS JNTS_RT_DONE_THIS," +
                      $"SUM(FNC_BETWEEN_DATE('{DATE_FROM}', '{DATE_TO}', CREATE_DATE, REP_LEN)) AS JNTS_REPAR_THIS," +
                      $"SUM(FNC_BETWEEN_DATE(TO_DATE('{DATE_FROM}') -7, TO_DATE('{DATE_FROM}') - 1, CREATE_DATE, WELD_LEN)) AS JNTS_WELDED_LAST_WEEK," +
                      $"SUM(FNC_BETWEEN_DATE(TO_DATE('{DATE_FROM}') -7, TO_DATE('{DATE_FROM}') - 1, CREATE_DATE, RT_LEN)) AS JNTS_RT_DONE_LAST_WEEK," +
                      $"SUM(FNC_BETWEEN_DATE(TO_DATE('{DATE_FROM}') -7, TO_DATE('{DATE_FROM}') - 1, CREATE_DATE, REP_LEN)) AS JNTS_REPAR_LAST_WEEK," +
                      $"SUM(FNC_IIF_DATE(CREATE_DATE, TO_DATE('{DATE_FROM}' )-7, 0, WELD_LEN, 0)) AS JNTS_WELDED_PREV," +
                      $"SUM(FNC_IIF_DATE(CREATE_DATE, TO_DATE('{DATE_FROM}') -7, 0, RT_LEN, 0)) AS JNTS_RT_DONE_PREV," +
                      $"SUM(FNC_IIF_DATE(CREATE_DATE, TO_DATE('{DATE_FROM}') -7, 0, REP_LEN, 0)) AS JNTS_REPAR_PREV, LINE_CLASS FROM VIEW_WELDER_PERF_F_NEW " +
                      $"WHERE PROJECT_ID={1}  AND SC_ID = {SUB_CON_ID} " +
                      $"GROUP BY WELDER_ID, WELDER_NO, SC_ID, SUB_CON_NAME, PROJECT_ID, JOB_CODE, CLIENT_NAME, JOB_NAME, PROJ_DESC, REP_TITLE1, REP_TITLE2, LINE_CLASS  " +
                      $"ORDER BY SUB_CON_NAME, WELDER_NO,LINE_CLASS";
                ...
                using (OracleDataReader dr = cmd.ExecuteReader())
                {
                    while (dr.Read())
                    {
                        ...
                        welderPerformance.LINE_CLASS = dr["LINE_CLASS"].ToString();
        ...
        [WebMethod]
        [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
        public void GetWelderPerformanceLineClassLengthDetailsPDF(int ProjId, int SUB_CON_ID, string DATE_FROM, string DATE_TO)
        {
            Random random = new Random();
            int fileId = random.Next(1000, 9999);
            string fileName = $"WelderPerformanceLineClassLength_{fileId}.pdf";
