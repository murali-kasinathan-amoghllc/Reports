...
{
    /// <summary>
    /// Summary description for WelderPerformanceRTPercentService
    /// </summary>
    [WebService(Namespace = "http://maakuthari.com/")]
    [WebServiceBinding(ConformsTo = WsiProfiles.BasicProfile1_1)]
    [System.ComponentModel.ToolboxItem(false)]
    [System.Web.Script.Services.ScriptService]
    public class WelderPerformanceRTPercentService : System.Web.Services.WebService
    {

        [WebMethod]
        [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
        public void GetWelderPerformanceRTPercentDetails(int ProjId, int SUB_CON_ID, string DATE_FROM, string DATE_TO)
        {
            ...
            using (OracleConnection conn = new OracleConnection(conn_str))
            {
                ...
                string sql = "SELECT PROJECT_ID, JOB_CODE, CLIENT_NAME, JOB_NAME, PROJ_DESC, " +
                      $"SC_ID, SUB_CON_NAME, RT, WELDER_ID, WELDER_NO," +
                      ...
                      $"GROUP BY WELDER_ID, WELDER_NO, SC_ID, SUB_CON_NAME, PROJECT_ID, JOB_CODE, CLIENT_NAME, JOB_NAME, PROJ_DESC, RT " +
                      $"ORDER BY SUB_CON_NAME, RT, WELDER_NO";
                ...
                using (OracleDataReader dr = cmd.ExecuteReader())
                {
                    while (dr.Read())
                    {
                        ...
                        string rt = dr["RT"].ToString();
                        if (!string.IsNullOrEmpty(rt))
                        {
                            welderPerformance.RT = Convert.ToInt32(dr["RT"].ToString());
                        }
        ...
        [WebMethod]
        [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
        public void GetWelderPerformanceRTPercentDetailsPDF(int ProjId, int SUB_CON_ID, string DATE_FROM, string DATE_TO)
        {
            Random random = new Random();
            int fileId = random.Next(1000, 9999);
            string fileName = $"WelderPerformanceRTPercent_{fileId}.pdf";
