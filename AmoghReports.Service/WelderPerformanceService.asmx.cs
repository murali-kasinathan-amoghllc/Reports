...
{
    /// <summary>
    /// Summary description for WelderPerformanceService
    /// </summary>
    [WebService(Namespace = "http://maakuthari.com/")]
    [WebServiceBinding(ConformsTo = WsiProfiles.BasicProfile1_1)]
    [System.ComponentModel.ToolboxItem(false)]
    [System.Web.Script.Services.ScriptService]
    public class WelderPerformanceService : System.Web.Services.WebService
    {

        [WebMethod]
        [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
        public void GetWelderPerformanceDetails(int ProjId, int SUB_CON_ID, string DATE_FROM, string DATE_TO)
        {
            ...
            using (OracleConnection conn = new OracleConnection(conn_str))
            {
                ...
                string sql = "SELECT PROJECT_ID, JOB_CODE, CLIENT_NAME, JOB_NAME, PROJ_DESC, " +
                      ...
                      $"ORDER BY SUB_CON_NAME, WELDER_NO,REP_TITLE1, REP_TITLE2";
        ...
        [WebMethod]
        [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
        public void GetWelderPerformanceDetailsPDF(int ProjId, int SUB_CON_ID, string DATE_FROM, string DATE_TO)
        {
            Random random = new Random();
            int fileId = random.Next(1000, 9999);
            string fileName = $"WelderPerformance_{fileId}.pdf";
