...
{
    /// <summary>
    /// Summary description for WelderPerformanceLengthWiseService
    /// </summary>
    [WebService(Namespace = "http://maakuthari.com/")]
    [WebServiceBinding(ConformsTo = WsiProfiles.BasicProfile1_1)]
    [System.ComponentModel.ToolboxItem(false)]
    [System.Web.Script.Services.ScriptService]
    public class WelderPerformanceLengthWiseService : System.Web.Services.WebService
    {

        [WebMethod]
        [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
        public void GetWelderPerformanceLengthWiseDetails(int ProjId, int SUB_CON_ID, string DATE_FROM, string DATE_TO)
        {
            ...
            using (OracleConnection conn = new OracleConnection(conn_str))
            {
                ...
                string sql = "SELECT PROJECT_ID, JOB_CODE, CLIENT_NAME, JOB_NAME, PROJ_DESC, " +
                      ...
                      $"SUM(FNC_IIF_DATE(CREATE_DATE, TO_DATE('{DATE_FROM}') -7, 0, REP_LEN, 0)) AS JNTS_REPAR_PREV FROM VIEW_WELDER_PERF_F " +
                      $"WHERE PROJECT_ID={1}  AND SC_ID = {SUB_CON_ID} " +
                      $"GROUP BY WELDER_ID, WELDER_NO, SC_ID, SUB_CON_NAME, PROJECT_ID, JOB_CODE, CLIENT_NAME, JOB_NAME, PROJ_DESC, REP_TITLE1, REP_TITLE2 " +
        ...
        [WebMethod]
        [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
        public void GetWelderPerformanceLengthWiseDetailsPDF(int ProjId, int SUB_CON_ID, string DATE_FROM, string DATE_TO)
        {
            Random random = new Random();
            int fileId = random.Next(1000, 9999);
            string fileName = $"WelderPerformanceLengthWise_{fileId}.pdf";
