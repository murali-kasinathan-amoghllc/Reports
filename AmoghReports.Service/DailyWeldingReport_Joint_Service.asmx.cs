...
{
    /// <summary>
    /// Summary description for DailyWeldingReport_Joint_Service
    /// </summary>
    [WebService(Namespace = "http://maakuthari.com/")]
    [WebServiceBinding(ConformsTo = WsiProfiles.BasicProfile1_1)]
    [System.ComponentModel.ToolboxItem(false)]
    [System.Web.Script.Services.ScriptService]
    public class DailyWeldingReport_Joint_Service : System.Web.Services.WebService
    {

        [WebMethod]
        [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
        public void GetDailyWeldingReportJointDetails(int PROJECT_ID, string SUB_CON_ID, string CAT_ID, string MAT_TYPE, string DATE_FROM, string DATE_TO)
        {
            ...
            using (OracleConnection conn = new OracleConnection(conn_str))
            {
                ...
                if (CAT_ID == "1")
                {
                    ...
                    $" WHERE PROJECT_ID={1} AND (SUB_CON_ID = '{SUB_CON_ID}' OR '{SUB_CON_ID}' ='99') AND (CAT_ID= '{1}' OR CAT_ID='{3}') AND (WELD_DATE >= '{DATE_FROM}') AND (WELD_DATE <= '{DATE_TO}') AND (MAT_TYPE = '{MAT_TYPE}' OR '{MAT_TYPE}' = 'XXX') ORDER BY ISO_TITLE1, SPOOL, JOINT_NO";
                    sql = categoryType;
                }
                else
                {
                    string categoryType = "SELECT * FROM VIEW_WELDING_REPORT" +
                    $" WHERE PROJECT_ID={1} AND (SUB_CON_ID = '{SUB_CON_ID}' OR '{SUB_CON_ID}' ='99') AND (CAT_ID= '{2}' OR CAT_ID='{4}') AND (WELD_DATE >= '{DATE_FROM}') AND (WELD_DATE <= '{DATE_TO}') AND (MAT_TYPE = '{MAT_TYPE}' OR '{MAT_TYPE}' = 'XXX') ORDER BY ISO_TITLE1, SPOOL, JOINT_NO";
                    sql = categoryType;
                }
        ...
        [WebMethod]
        [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Json)]
        public void GetDailyWeldingReportJointDetailsPDF(int PROJECT_ID, string SUB_CON_ID, string CAT_ID, string MAT_TYPE, string DATE_FROM, string DATE_TO)
        {
            Random random = new Random();
            int fileId = random.Next(1000, 9999);
            string fileName = $"DailyWeldingReport_{fileId}.pdf";
